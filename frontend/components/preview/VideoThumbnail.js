    ...
    <div className="flex flex-row gap-x-2 overflow-x-auto w-full">
      {Object.entries(images).map(([modelName, imageUrl]) => (
        <div key={modelName} title={modelName} className="">
          <Image width={150} height={100} className="min-w-[150px] w-auto h-auto rounded-lg" onClick={() => setSelectedModel(modelName)} key={modelName} src={imageUrl} alt={`Thumbnail for ${modelName}`} loading="lazy" />
