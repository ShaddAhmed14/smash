    ...
    <div className="max-w-full h-9/10 flex flex-col m-4">
      <div className="mb-2 flex flex-row gap-4">
        <h2 onClick={() => setOption('pertalk')} className={`plot-option-pill hover:bg-primary
          ${option === 'pertalk' ? 'bg-primary font-semibold border-2 border-[color:var(--button-primary)]' : 'bg-secondary border-[color:var(--bg-primary)]'}`}>Per-Talk</h2>
        <h2 onClick={() => setOption('crosstalk')} className={`plot-option-pill hover:bg-primary
          ${option === 'crosstalk' ? 'bg-primary font-semibold border-2 border-[color:var(--button-primary)]' : 'bg-secondary border-[color:var(--bg-primary)]'}`}>Cross-Talk</h2>
      </div>
      {
        option === 'crosstalk' ?
        <div className="h-full flex flex-row justify-around">
          <div className="w-full h-full m-2 bg-primary flex flex-col">
            <p className="font-semibold text-[0.9375rem] border-b-2 border-[color:var(--custom-analytics-dark)] py-3 px-4">{"TFIDF"}</p>
            <SemanticNetworkTemplate type={"TFIDF"} />
          </div>
          <div className="w-full h-full m-2 bg-primary flex flex-col">
            <p className="font-semibold text-[0.9375rem] border-b-2 border-[color:var(--custom-analytics-dark)] py-3 px-4">{"SBERT"}</p>
