...
def read_srt_files(folder_path):
    """Reads all .srt files from a folder and returns a list of dictionaries."""
    data = []
    for filename in os.listdir(folder_path):
        if filename.endswith(".srt"):
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                data.append({'title': filename, 'transcript': content})
    return data

def clean_srt_transcript(transcript):
    """Removes timestamps, index numbers, and empty lines from an SRT transcript."""
...
def main_bertopic(video_transcripts, video_titles):
    docs = []
    titles = []
...
def video_cluster( document_topic_probabilities, titles):
    ...
    with open('video_plot_data.json', 'w') as f: # Video Clusters based on Topic Distribution
...
def topic_interdistance(topic_model, video_topics_df):
    ...
    with open('topic_plot_data.json', 'w') as f: # Intertopic Distance Map
...
def data_map(topic_model, embeddings, titles):
    ...
    with open('datamap_data.json', 'w') as f:
...
def temporal_sentiment_analysis(df):
    ...
    with open('temporal_sentiment_data.json', 'w') as f:
        json.dump(temporal_sentiment_data, f, indent=4)


if __name__ == "__main__":
    srt_folder = 'transcripts'
    srt_data = read_srt_files(srt_folder)
