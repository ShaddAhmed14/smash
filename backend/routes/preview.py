...
def find_video_name(video_id: str) -> Optional[str]:
    """Find video folder name matching the given video_id prefix."""
...
async def stream_audio(video_name: str):
    ...
    if not os.path.exists(file_path):
        return JSONResponse(content={"message": "Audio file not found"}, status_code=404)
...
async def fetch_thumbnails(video_name: str):
    """Return base64-encoded thumbnails for all pose models."""
    video_name = find_video_name(video_name)
    if not video_name:
        return JSONResponse(content={"message": "Video not found"}, status_code=404)
...
async def fetch_thumbnail(video_name: str):
    """Return the Original thumbnail image for a single video."""
    video_name = find_video_name(video_name)
    if not video_name:
        return JSONResponse(content={"message": "Video not found"}, status_code=404)
    file_path = os.path.join(MATERIALS_FOLDER, video_name, "thumbnails")
    ...
    if not os.path.exists(img_file):
        return JSONResponse(content={"message": "Thumbnail not found"}, status_code=404)
...
async def fetch_audio_peaks(video_name: str):
    ...
    if not os.path.exists(file_path):
        logger.warning("Audio peaks file not found: %s", file_path)
